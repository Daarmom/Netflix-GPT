...
import { addUser, removeUser } from '../utils/store/userSlice.js';
import { toggleGptSearchView } from '../utils/store/gptSlice.js';
import { changeLanguage } from '../utils/store/configSlice.js';
import {LOGO, NETFLIX_LOGO, SUPPORTED_LANGUAGES} from '../utils/constants/constants.js'
...
const NavDesktop = () => {
  ...
  return (
    <div className='absolute w-screen px-8 py-2 bg-gradient-to-b from-black z-10 flex flex-col md:flex-row  md:justify-between'>
      ...
      {user && <ul className='hidden md:flex p-2 flex justify-between'>
        <li>
          {showGptSearch && (
            <select
        ...
        <li>
          <img className="hidden md:block py-2 w-10 h-14" alt='UserIcon' src={LOGO}></img>
        </li>
